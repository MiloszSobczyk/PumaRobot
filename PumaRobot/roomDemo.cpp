	...
	m_particles{ {-1.3f, -0.6f, -0.14f} }
{
	...
	// DONE : 0.01 Set to proper addressing (wrap) and filtering (16x anisotropic) modes of the sampler
	...
	// DONE : 1.06 Initialize second sampler state
	...
	// DONE : 1.10 Moddify MipLODBias field for the second sampler
	sd.MipLODBias = 0.5f;
	...
	// DONE : 1.08 Calculate correct transformation matrix for the poster texture
...
void RoomDemo::DrawWalls()
{
	...
	// DONE : 0.02 calculate texture tranformation matrix for walls/ceiling/floor that transforms rectangle [-2,-2]x[2,2] into [0,0]x[1,1]
	...
	// DONE : 0.03 set shaders to m_textureVS, m_texturePS
	// DONE : 0.11 draw ceiling with colorTexture pixel shaders instead
	SetShaders(m_textureVS, m_colorTexPS);

	// DONE : 0.04 set ceiling the texture to perlin noise
	...
	// DONE : 0.05 set the walls' texture to brick wall 
	//SetTextures({ m_wallTexture.get() });

	// DONE : 0.12 draw back walls with regular texture shaders
	//SetShaders(m_textureVS, m_texturePS);
	DrawMesh(m_wall, m_wallsMtx[0]);

	//draw remainting walls
	// DONE : 0.13 draw remaining walls with regular texture shaders
	SetShaders(m_textureVS, m_texturePS);
	for (auto i = 1; i < 4; ++i)
		DrawMesh(m_wall, m_wallsMtx[i]);

	//draw floor
	// DONE : 0.09 Change texture matrix to stretch floor texture 8 times along y, i.e. transform rectangle [-2,-2]x[2,2] into [0,0]x[1,16]. Run the program again to see the difference.
	...
	// DONE : 0.06 set floor texture to wood
...
void RoomDemo::DrawTeapot()
{
	...
	// DONE : 1.26 [optional] Comment the following line and uncomment the next to replace teapot with a sphere
...
void RoomDemo::Render()
{
	...
	// DONE : 1.21 Set evnMapper render target
